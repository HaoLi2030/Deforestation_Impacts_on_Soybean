...
def main_attribution(
    ...
    # get start date (to read trajectories) - NOTE: we begin at first time step after 00 UTC...
    sdate_bgn = (
        str(ayyyy)
        + "-"
        + str(am).zfill(2)
    ...
    if mode == "test":
        ...
        ntime = nsteps_per_day  # NOTE: use multiples of nsteps_per_day (e.g. 4 for dt=6) only, else output is not saved
        datetime_seq = datetime_seq[(nsteps_per_day * ctraj_len) : (nsteps_per_day * ctraj_len + ntime)]
        fdatetime_seq = fdatetime_seq[(nsteps_per_day * ctraj_len) : (nsteps_per_day * ctraj_len + ntime)]
        ndaytime = int(ntime / nsteps_per_day)
    ...
    # traj max len, expressed in input data dt steps
    tml = int(nsteps_per_day * ctraj_len)
    ...
    if fmemento:
        pidlog = -999 * np.ones(shape=tnparcel).astype(int)
        ...
        if mode == "oper":  # skip if multi-counting somehow desired and/or if testing
            ...
            if ntrajstep < tml + 2 + nsteps_per_day:
    ...
    for ix in range(ntime):
        ...
        arv_idx = np.where(
            ...
            == (fdatetime_seq[ix] - relativedelta(hours=dt/2)).date()
            # JK check if this works for 0.5 h
        )[0][0]

        # pre-allocate arrays (repeat at every nsteps_per_day step)
        if ix % nsteps_per_day == 0:
        ...
        if fmemento:
            ...
            if ary.shape[0] < tml + 2 + nsteps_per_day and ix < ctraj_len * nsteps_per_day:
                extendtrajs = np.empty(shape=(nsteps_per_day, nparcel, 2))
                for pp in range(nsteps_per_day):
                    allIDs = extendarchive[-(nsteps_per_day - pp + ix)][:, 0]
                    extendtrajs[pp, :, 0] = extendarchive[-(nsteps_per_day - pp + ix)][:, 0][
                        np.where(np.isin(allIDs, ary[0, :, 0]))
                    ]  # ID
                    extendtrajs[pp, :, 1] = extendarchive[-(nsteps_per_day - pp + ix)][:, 1][
                        np.where(np.isin(allIDs, ary[0, :, 0]))
                    ]  # hpbl
        ...
        for i in ntot:
            ...
            if mask[alat_ind, alon_ind] == maskval:
                ID = int(ary[0, i, 0])
                if fmemento and ary[0, i, 3] < np.max(ary[:nsteps_per_day, i, 7]):
                    if ix < ctraj_len * nsteps_per_day:  # rely on (extended) traj data
                        ...
                        if ary.shape[0] < tml + 2 + nsteps_per_day:
                            longhpbl = np.concatenate(
                                (ary[: (tml + 2), i, 7], extendtrajs[:, i, 1])
                            )
                        else:
                            longhpbl = ary[: (tml + 2 + nsteps_per_day), i, 7]
                        is_inpbl = np.where(hgt < maxlastn(longhpbl, n=nsteps_per_day)[:-nsteps_per_day])[
                            0
                        ]  # omit last day
            ...
            pdate = str(
                (fdatetime_seq[ix] - relativedelta(hours=dt/2)).strftime("%Y%m%d%H")
                # JK: check if this works for 0.5 hours
            )
            ...
            if not mask[mlat_ind, mlon_ind] == maskval:
                ...
                if (qv[0] - qv[1]) < cprec_dqv and (
                    ...
                    if evap_idx.size > 0:
                        ...
                        # write to grid
                        for itj in evap_idx:
                            ary_etop[ctl - (itj + (nsteps_per_day-1) - ix % nsteps_per_day) // nsteps_per_day, :, :] += gridder(
            ...
            if not mask[alat_ind, alon_ind] == maskval:
                ...
                if ihf_H >= 2 and hgt[0] < np.max(hpbl[:nsteps_per_day]):
                    ...
                    for itj in heat_idx:
                        ...
                        ary_heat[ctl - (itj + (nsteps_per_day-1) - ix % nsteps_per_day) // nsteps_per_day, :, :] += (
        ...
        if (ix + 1) % nsteps_per_day == 0:
            ...
            ary_etop[:, :, :] = convertunits(ary_etop[:, :, :], garea, pmass, dt, "E")
            ary_heat[:, :, :] = convertunits(ary_heat[:, :, :], garea, pmass, dt, "H")
