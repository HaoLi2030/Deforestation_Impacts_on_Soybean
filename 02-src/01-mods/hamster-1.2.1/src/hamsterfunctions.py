...
# 3 million, globally
#PMASS = 1699160060371.808  # average parcel mass
...
def read_cmdargs():
    ...
    parser.add_argument(
        "--dt", "-dt", help="time step (hours)", metavar="", type=int, default=6
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--fisgz",
        "-fgz",
        help="f2t: partposit files are gz files (True) or binary files (False)",
        metavar="",
        type=str2bol,
        default=True,
        nargs="?",
    ...
    parser.add_argument(
        "--fanndir",
        "-fad",
        help="data is structured in annual subfolders",
        metavar="",
        type=str2bol,
        default=True,
        nargs="?",
    ...
    parser.add_argument(
        "--maxparcel",
        "-mp",
        help="total (max.) number of parcels being read and evaluated",
        metavar="",
        type=int,
        default=2000000,
    )
    parser.add_argument(
        "--parcelmass",
        "-pmass",
        help="average parcel mass in kg",
        metavar="",
        type=float,
        default=1699160060371.808,
    )
...
def convertunits(ary_val, garea, pmass, dt_h, var):
    """
    INPUT
        - aryval
    ACTION
    ...
    if var in ["H"]:
        return pmass * ary_val * CPD / (1e6 * garea * dt_h * 3600)
...
def eraloader_12hourly(
    ...
    elif var == "slhf" and units == "J m**-2":
        ilambda              = 2.4501 * 1e6 # J kg-1
        #print(" *** value range before conversion: "+str(np.min(daily))+" - "+str(np.max(daily)))
        daily /= -(ilambda*(3600*24)*1000)  # flip sign
        #print(" *** value range after conversion: "+str(np.min(daily))+" - "+str(np.max(daily)))
    else:
...
def convert2daily(xar, ftime, nsteps_per_day=4, fagg="mean"):
...
def f2t_read_partposit(ifile, maxn=3e6, verbose=False):
    ...
    #nbytes_per_parcel = 8 + 4 + 9 * 4  # reduced partposit output
    #nbytes_per_parcel = 8 + 4 + 12 * 4 # full partposit output
    if "gz" in ifile:
        with gzip.open(ifile, "rb") as strm:
            ...
            # decode binary data
            pdata = struct.unpack(
                (nparc) * binstr, tdata[0 : ((nparc) * nbytes_per_parcel)]
                #(nparc) * "2fi3fi8f", tdata[0 : ((nparc) * nbytes_per_parcel)] # full partposit
                #(nparc) * "2fi9f", tdata[0 : ((nparc) * nbytes_per_parcel)]    # reduced partposit
            )
            flist = list(pdata)
        strm.close()
    else:
        with open(ifile, "rb") as strm:
            ...
            nparc = math.floor(len(tdata) / (nbytes_per_parcel))
            print(" ")
            print(" \t\t Number of parcels (estd. from bytes in file): "+str(nparc))
            # decode binary data
            pdata = struct.unpack(
                #(nparc) * "2fi3fi8f", tdata[0 : ((nparc) * nbytes_per_parcel)] # full partposit
                #(nparc) * "2fi9f", tdata[0 : ((nparc) * nbytes_per_parcel)]    # reduced partposit
            ...
            flist = list(pdata)
        strm.close()
    #pdata = np.reshape(flist, newshape=(nparc, 15))[:, 2:] # full partposit
    #pdata = np.reshape(flist, newshape=(nparc, 12))[:, 2:] # reduced partposit
    ...
    #print(" ")
    #print("pid="+str(pdata[0:10,0]))
    #print("temp="+str(pdata[0:10,9]))  # reduced partposit
    #print("temp="+str(pdata[0:10,11])) # full partposit
    #print("x="+str(pdata[0:10,1]))
    #print("y="+str(pdata[0:10,2]))
    #print("z="+str(pdata[0:10,3]))
    return pdata
...
def f2t_loader(ifile, fixlons=True, fixids=False, verbose=True):
...
def check_for_duplicates(IDs, verbose):
...
def f2t_fixid(IDs, verbose, thresidx=1997000):
    dup = check_for_duplicates(IDs, verbose)
    if len(dup)>0:
        ## ATTN: this is hardcoded for a specific run!!!
        ## may set fixid=False everywhere, to avoid doing this
...
def f2t_establisher(
    partdir, selvars, time_str, ryyyy, mask, maskval, mlat, mlon, outdir, fout, fisgz, tnparcel, verbose
):
    ##-- 1.) load em files
    data = np.empty(shape=(len(time_str), tnparcel+1, selvars.size))
    for ii in range(len(time_str)):
        ...
        if fisgz:    
            ...
            ifile = partdir + "/partposit_" + time_str[ii]
        dummy = f2t_loader(ifile, fixlons=True, fixids=False, verbose=verbose)[:, selvars]  # load
...
def f2t_ascender(
    ...
    if fisgz:
        ...
        ifile = partdir + "/partposit_" + time_str[-1]
    dummy = f2t_loader(ifile, fixlons=True, fixids=False, verbose=verbose)[:, selvars]
